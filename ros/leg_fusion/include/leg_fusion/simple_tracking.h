...
public:
  ...
  void addDetectorModel(std::string name, association_t alg, observ_model_t om_flag, double pos_noise_x,
                        double pos_noise_y, unsigned int seqSize = 5, double seqTime = 0.2)
  {
                                                          ...
                                                          bool publish_occluded)
  {
    ...
    // Since the update step is related to observations (from detectors), it is performed in addObservation function,
    // when a new observation comes
    // for(typename std::map<std::string, detector_model>::const_iterator it = detectors.begin(); it != detectors.end();
    //    ++it) {
    //    // prediction
    //    cvm->update(dt);
    ...
    for (int i = 0; i < mtrk.size(); i++)
    {
      if (publish_occluded || mtrk[i].filter->is_matched)
      {
        double theta = atan2(mtrk[i].filter->x[3], mtrk[i].filter->x[1]);
        ROS_DEBUG("trk_%ld: Position: (%f, %f), Orientation: %f, Std Deviation: %f, %f", mtrk[i].id,
                  mtrk[i].filter->x[0],
                  mtrk[i].filter->x[2],                                         // x, y
                  theta,                                                        // orientation
                      ...
                      std::vector<std::string> tags)
  {
    ...
    for (int i = 0; i < mtrk.size(); i++)
    {
      mtrk[i].filter->is_matched = false;
    }
