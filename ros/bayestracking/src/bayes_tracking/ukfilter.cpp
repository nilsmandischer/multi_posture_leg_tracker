...
UKFilter::UKFilter(std::size_t x_size)
  : Kalman_state_filter(x_size), Unscented_scheme(x_size), z_p(Empty), is_matched(true)
{
...
UKFilter::UKFilter(const FM::Vec& x0, const FM::SymMatrix& P0)
  : Kalman_state_filter(x0.size()), Unscented_scheme(x0.size()), z_p(Empty), is_matched(true)
{
...
void UKFilter::update(Additive_predict_model& predict_model, Correlated_additive_observe_model& observe_model,
                      const FM::Vec& z)
{
...
void UKFilter::predict_observation(Correlated_additive_observe_model& observe_model, FM::Vec& z_pred,
                                   FM::SymMatrix& R_pred)
{
