...
EKFilter::EKFilter(std::size_t x_size)
  : Kalman_state_filter(x_size), Covariance_scheme(x_size), s(Empty), z_p(Empty), is_matched(true)
{
...
EKFilter::EKFilter(const FM::Vec& x0, const FM::SymMatrix& P0)
  : Kalman_state_filter(x0.size()), Covariance_scheme(x0.size()), s(Empty), z_p(Empty), is_matched(true)
{
...
void EKFilter::update(Linrz_predict_model& predict_model, Linrz_correlated_observe_model& observe_model,
                      const FM::Vec& z)
{
...
Bayes_base::Float EKFilter::observeInnovation(Linrz_correlated_observe_model& h, const Bayesian_filter_matrix::Vec& si,
