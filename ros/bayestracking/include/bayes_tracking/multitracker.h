...
public:
  ...
  /**
   * Perform data association and update step for all the current filters, create new ones and remove those which are no
   * more necessary
...
private:
  ...
  bool dataAssociation(ObservationModelType& om, association_t alg = NN)
  {
    ...
    {  // observations and tracks, associate
      ...
      for (int j = 0; j < N; j++)
      {
        ...
        for (int i = 0; i < M; i++)
        {
          ...
          if (alg == NN_LABELED || alg == NNJPDA_LABELED)
          {
            ...
            if (m_observations[i].tag.length() > 0 && m_filters[j].tag.length() > 0)
            {
      ...
      else if (alg == NNJPDA || alg == NNJPDA_LABELED)
      {  /// NNJPDA data association (one sensor)
...
private:
  ...
  void createTracks(ObservationModelType& om, unsigned int seqSize, double seqTime, observ_model_t om_flag)
  {
    ...
    while (ui != m_unmatched.end())
    {
      ...
      while (si != m_sequences.end())
      {
        ...
        else if (AM::mahalanobis(m_observations[*ui].vec, om.Z, si->back().vec, om.Z) <= AM::gate(om.z_size))
  ...
  void observe(ObservationModelType& om)
  {
    ...
    for (ai = m_assignments.begin(); ai != aiEnd; ai++)
    {
      ...
      m_filters[ai->second].filter->is_matched = true;
