#include "local_occupancy_grid_mapping.h"
...
{
OccupancyGridMapping::OccupancyGridMapping(ros::NodeHandle nh, std::string scan_topic)
  : nh_(nh)
  , grid_centre_pos_found_(false)
  , scan_topic_(scan_topic)
  , scan_sub_(nh_, scan_topic, 100)
  , non_leg_clusters_sub_(nh_, "/laser_non_person_clusters", 100)
...
void OccupancyGridMapping::laserAndLegCallback(const sensor_msgs::LaserScan::ConstPtr& scan_msg,
                                               const rc_tracking_msgs::LegArray::ConstPtr& non_leg_clusters)
{
  ...
  if (transform_available)
  {
    ...
    {
      rc_tracking_msgs::Leg leg = non_leg_clusters->legs[i];
    ...
    laser_human_tracker::laser_processor::ScanProcessor processor(scan);
    processor.splitConnected(cluster_dist_euclid_);
    processor.removeLessThan(min_points_per_cluster_);
    for (std::list<laser_human_tracker::laser_processor::SampleSet*>::iterator c_iter = processor.getClusters().begin();
         c_iter != processor.getClusters().end(); ++c_iter)
    {
      ...
      for (laser_human_tracker::laser_processor::SampleSet::iterator s_iter = (*c_iter)->begin();
           s_iter != (*c_iter)->end(); ++s_iter)
      {
    ...
    if (transform_succesful)
    {
      ...
      for (int i = 0; i < width_; i++)
      {
        for (int j = 0; j < width_; j++)
        {
          ...
          double angle =
              betweenPIandNegPI(atan2(j * resolution_ + grid_centre_pos_y_ - (width_ / 2.0) * resolution_ - laser_y,
              ...
              angle < scan.angle_max + scan.angle_increment / 2.0)
          {
            ...
            int closest_beam_idx =
                std::max(0, std::min(static_cast<int>(scan.ranges.size() - 1), idx_without_bounds_check));
            is_human = is_sample_human[closest_beam_idx];

            // Processing the range value of the closest_beam to determine if
            // it's a valid measurement or not. Sometimes it returns infs and
            // NaNs that have to be dealt with
...
double OccupancyGridMapping::logit(double p)
{
  return log(p / (1 - p));
}

double OccupancyGridMapping::inverseLogit(double p)
{
  return exp(p) / (1 + exp(p));
}

double OccupancyGridMapping::betweenPIandNegPI(double angle_in)
{
