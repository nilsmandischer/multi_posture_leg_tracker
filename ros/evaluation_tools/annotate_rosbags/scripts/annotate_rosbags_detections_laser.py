...
class AnnotateRosbagsDetections:
    def __init__(self):  
        ...
        self.key_c_pressed_sub = rospy.Subscriber("key_c_pressed", Char, self.key_c_pressed_callback)
...
#        if self.finished_successfully and not self.display_mode:
    ...
    # While using selected_points_publisher tool in rviz, press key n to go to next scan.
    def key_n_pressed_callback(self, msg):
        self.nextScanMsg()

    # While using selected_points_publisher tool in rviz, press key c to clean current cluster array.
    def key_c_pressed_callback(self, msg):
       self.cluster_pose_array = None
    ...
    def nextScanMsg(self, num_scan_msgs=1, display=True):
        ...
        if self.cluster_pose_array:
            ...
            current_scan = self.cur_scan_msg
            current_scan.header.frame_id = self.fixed_frame
            self.cur_scan_msg = None
            self.savebag.write("/training_scan", current_scan, save_time)
            self.cluster_pose_array = None
