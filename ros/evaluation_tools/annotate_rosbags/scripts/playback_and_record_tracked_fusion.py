...
from rc_tracking_msgs.msg import Person, PersonArray
...
class PlayBackAndRecordTracked:    
    def __init__(self):  
        ...
        self.laser_scan_topic = rospy.get_param("laser_scan_topic", None)
        self.radar_scan_topic = rospy.get_param("radar_scan_topic", None)
        self.record_logs = rospy.get_param("record_logs", False)

        if self.readbag_filename is None or self.laser_scan_topic is None or self.radar_scan_topic is None:
        ...
        self.people_tracked_sub = rospy.Subscriber('/fused_tracked_people', PersonArray, self.people_tracked_callback)
    ...
    def close_bags(self, not_using_this_var=None):
        ...
        if self.tracking_msg_count != 0:
            rospy.loginfo("avg tracking time: %.2fms worst: %.2fms based on %d msgs", self.tracking_time_cum * 10**3 / float(self.tracking_msg_count), self.worst_tracking_time * 10**3, self.tracking_msg_count)
    ...
    def people_tracked_callback(self, people_tracked_msg):    
        ...
        if self.tracking_started:
            ...
            if tracking_time > self.worst_tracking_time:
                ...
                rospy.loginfo("***************************Worst tracking time so far: %.2f***************************", self.worst_tracking_time * 10**3)
        ...
        if self.savebag_filename is not None:
            ...
            self.savebag.write('/fused_tracked_people', people_tracked_msg, save_time)
        ...
        self.msg_num += 1
        rospy.loginfo("Received Msgs so far: %d", self.msg_num)
...
#                self.radar_pub.publish(msg)
        ...
        marker_id = 0
        topic = None
        while topic != self.laser_scan_topic and topic != self.radar_scan_topic and not self.bag_empty:
            try:
                # Get the next message from the rosbag
                topic, msg, time = next(self.msg_gen)

                if topic == self.laser_scan_topic or topic == self.radar_scan_topic:
                    self.next_scan_msg = (topic, msg, time)
                    self.next_scan_msg_time = time

                    # publish laser or radar scan and start timer
                    if self.cur_scan_msg is not None:
                        if self.cur_scan_topic == self.laser_scan_topic:
                            self.laser_pub.publish(self.cur_scan_msg)
                        else:
                            self.radar_pub.publish(self.cur_scan_msg)
                        self.tic = timeit.default_timer()
                        self.cur_scan_msg = None
                else:
                    if topic == "/tf":
                        ...
                        if self.savebag_filename is not None:
                            self.savebag.write(topic, msg, time)
                    elif topic == "/radar_ground_truth_people_tracks" or topic == "/laser_ground_truth_people_tracks":
                        if self.savebag_filename is not None:
                            self.savebag.write(topic, msg, time)
        ...
        if self.prev_marker_id > marker_id:
            for m_id in range(marker_id, self.prev_marker_id):
                marker = Marker()
